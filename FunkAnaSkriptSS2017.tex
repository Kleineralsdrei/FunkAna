	...
	\begin{cor}
		$(X,d)$ sei ein metrischer Raum. Dann gilt:
			\begin{enumerate}[a)]
				...
				\item $M\subseteq X$, so dass jede Folge in M eine in M konvergente Teilfolge hat (\afs M folgenkomapkt\afs)
					$\Leftrightarrow$ $M \subseteq X$ kompakt (\afs M Überdeckungskompakt\afs)
...
%Lemma 1.47 Lemma von Riesz
	...
	\begin{proof}
		1. \afs $\Leftarrow$ \afs Folgt aus Heine-Borel\par
		 \par
...
\newcommand{\xf}{(x_i)_{i\in I}}
	...
	\begin{bsp}
		\begin{enumerate}[a)]
			...
			Beachte: $V^\perp$ immer abgeschlossen, da für eine Folge $(v_i)$ in $V^\perp$ mit $v_i \to v$ gilt $(x,v) \leftarrow (x,v_i) = 0 \; \forall x\in V$ $\df v \in V^\perp$
		\end{enumerate}
	\end{bsp}
